...
valid_labels = set()
with open('valid-issue-labels-master.txt','r') as f:
    for line in f.readlines():
        label = line.strip()
...
for repo in session.query(Repo):
    ...
    if(is_valid):
        ...
        num_raw_issues = 0
        for issue in issues:
            if(issue.labels in valid_labels):
                num_raw_issues +=1 
    ...
    num_issuelinks = session.query(IssueLink).filter_by(repo_id = repo.id).count()
    ...
    db_proj = Project(name = repo.slug, age = age, repo_id_lind = repo_id_Lind, 
        ...
        user_type = user_type, repo_id_GHT = repo_id_GHT, num_issuelinks = num_issuelinks, url = url_Lind, num_bugs = num_bugs, 
        is_treatment=  is_treatment, num_issues = num_issues, num_raw_issues = num_raw_issues, num_PRs = num_PRs, 
        num_forks = num_forks)
    session.add(db_proj)
    session.commit()
