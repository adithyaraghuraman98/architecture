...
from orm import User, SessionWrapper
...
def load_users_from_db():
    # create a new session without init'ing db tables
...
def main(argv):
    ...
    for user in users:
        ...
        idx += 1
        if idx >= curidx:
            logging.INFO(curidx/step, '/ 30')
            curidx += step

    logging.info('Done: helpers')
    ...
    logging.info('Done: comp email prefix')
    ...
    logging.info('Done: email prefix')
    ...
    logging.info('Done: prefix=login')
    ...
    logging.info('Done: full/simple name')
    ...
    logging.info('Done: email domain')
